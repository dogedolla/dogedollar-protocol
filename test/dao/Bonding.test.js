...
describe('Bonding', function () {
    ...
    beforeEach(async function () {
        this.bonding = await MockBonding.new({
            from: ownerAddress,
            gas: 8000000,
        })
    ...
    describe('frozen', function () {
        ...
        describe('when deposit', function () {
            beforeEach(async function () {
                ...
                await this.dollar.approve(this.bonding.address, 1000, {
                    from: userAddress,
                })

                this.result = await this.bonding.deposit(1000, {
                    from: userAddress,
                })
            ...
            it('updates users balances', async function () {
                ...
                expect(
                    await this.bonding.balanceOf(userAddress)
                ).to.be.bignumber.equal(new BN(0))
                expect(
        ...
        describe('when withdraw', function () {
            beforeEach(async function () {
                ...
                this.result = await this.bonding.withdraw(1000, {
                    from: userAddress,
                })
                this.txHash = this.result.tx
            })
        ...
        describe('when withdraw too much', function () {
            beforeEach(async function () {
                ...
                await this.dollar.approve(this.bonding.address, 10000, {
                    from: userAddress1,
                })
        ...
        describe('when bond', function () {
            describe('simple', function () {
                beforeEach(async function () {
                    ...
                    this.result = await this.bonding.bond(1000, {
                        from: userAddress,
                    })
                    this.txHash = this.result.tx
                })
                ...
                it('updates dao balances', async function () {
                    ...
                    expect(
                        await this.bonding.totalSupply()
                    ).to.be.bignumber.equal(
                        new BN(1000).mul(INITIAL_STAKE_MULTIPLE)
                    )
                    expect(
                        await this.bonding.totalBonded()
                    ).to.be.bignumber.equal(new BN(1000))
                    expect(
                        await this.bonding.totalStaged()
                    ).to.be.bignumber.equal(new BN(0))
                })
                ...
                it('emits Bond event', async function () {
                    ...
                    expect(event.args.value).to.be.bignumber.equal(
                        new BN(1000).mul(INITIAL_STAKE_MULTIPLE)
                    )
                    expect(event.args.valueUnderlying).to.be.bignumber.equal(
                        new BN(1000)
                    )
                })
            ...
            describe('partial', function () {
                beforeEach(async function () {
                    ...
                    this.result = await this.bonding.bond(500, {
                        from: userAddress,
                    })
                    this.txHash = this.result.tx
                })
                ...
                it('updates users balances', async function () {
                    ...
                    expect(
                        ...
                        new BN(500).mul(INITIAL_STAKE_MULTIPLE)
                    )
                    expect(
                        await this.bonding.balanceOfStaged(userAddress)
                ...
                it('updates dao balances', async function () {
                    expect(
                        await this.dollar.balanceOf(this.bonding.address)
                    ).to.be.bignumber.equal(new BN(800))
                    expect(
                        await this.bonding.totalSupply()
                    ).to.be.bignumber.equal(
                        new BN(500).mul(INITIAL_STAKE_MULTIPLE)
            ...
            describe('multiple', function () {
                beforeEach(async function () {
                    ...
                    await this.bonding.mintToE(userAddress, 1000)
                    await this.dollar.approve(this.bonding.address, 800, {
                        from: userAddress,
                    })
                    await this.bonding.deposit(800, { from: userAddress })
                ...
                it('updates users balances', async function () {
                    ...
                    expect(
                        ...
                        new BN(250).mul(INITIAL_STAKE_MULTIPLE)
                    )
                    expect(
                        await this.bonding.balanceOfStaged(userAddress)
                    ).to.be.bignumber.equal(new BN(300))
                ...
                it('updates dao balances', async function () {
                    expect(
                        await this.dollar.balanceOf(this.bonding.address)
                    ).to.be.bignumber.equal(new BN(3800))
                    expect(
                        await this.bonding.totalSupply()
                    ).to.be.bignumber.equal(
                        new BN(1250).mul(INITIAL_STAKE_MULTIPLE)
                    )
                    expect(
                        await this.bonding.totalBonded()
                    ).to.be.bignumber.equal(new BN(2500))
                    expect(
                        await this.bonding.totalStaged()
                    ).to.be.bignumber.equal(new BN(1300))
                })
        ...
        describe('when unbond', function () {
            ...
            describe('simple', function () {
                beforeEach(async function () {
                    this.result = await this.bonding.unbond(
                        new BN(1000).mul(INITIAL_STAKE_MULTIPLE),
                        { from: userAddress }
                    )
                    this.txHash = this.result.tx
            ...
            describe('partial', function () {
                beforeEach(async function () {
                    this.result = await this.bonding.unbond(
                        new BN(800).mul(INITIAL_STAKE_MULTIPLE),
                        { from: userAddress }
                    )
                    this.txHash = this.result.tx
                })
            ...
            describe('multiple', function () {
                ...
                it('updates users balances', async function () {
                    ...
                    expect(
                        await this.bonding.balanceOfStaged(userAddress)
                    ).to.be.bignumber.equal(new BN(1200))
                    expect(
                        await this.bonding.balanceOfBonded(userAddress)
                    ).to.be.bignumber.equal(new BN(300))
                })
                ...
                it('updates dao balances', async function () {
                    expect(
                        await this.dollar.balanceOf(this.bonding.address)
                    ).to.be.bignumber.equal(new BN(4000))
                    expect(
                        await this.bonding.totalSupply()
                    ).to.be.bignumber.equal(
                        new BN(1200).mul(INITIAL_STAKE_MULTIPLE)
                    )
                    expect(
                        await this.bonding.totalBonded()
                    ).to.be.bignumber.equal(new BN(1800))
                    expect(
                        await this.bonding.totalStaged()
                    ).to.be.bignumber.equal(new BN(2200))
                })
        ...
        describe('when unbondUnderlying', function () {
            ...
            describe('simple', function () {
                beforeEach(async function () {
                    this.result = await this.bonding.unbondUnderlying(1000, {
                        from: userAddress,
                    })
                    this.txHash = this.result.tx
                })
            ...
            describe('partial', function () {
                beforeEach(async function () {
                    this.result = await this.bonding.unbondUnderlying(800, {
                        from: userAddress,
                    })
                    this.txHash = this.result.tx
                })
            ...
            describe('multiple', function () {
                ...
                it('updates users balances', async function () {
                    ...
                    expect(
                        await this.bonding.balanceOf(userAddress)
                    ).to.be.bignumber.equal(new BN(466666667))
                    expect(
                        await this.bonding.balanceOfStaged(userAddress)
                    ).to.be.bignumber.equal(new BN(800))
                    expect(
                ...
                it('updates dao balances', async function () {
                    ...
                    expect(
                        await this.bonding.totalSupply()
                    ).to.be.bignumber.equal(new BN(1466666667))
                    expect(
                        await this.bonding.totalBonded()
                    ).to.be.bignumber.equal(new BN(2200))
                    expect(
    ...
    describe('locked', function () {
        ...
        describe('when bond', function () {
            it('reverts', async function () {
                await expectRevert(
                    this.bonding.bond(1000, { from: userAddress }),
                    'Permission: Not frozen'
                )
            })
        })
        ...
        describe('when unbond', function () {
            it('reverts', async function () {
                await expectRevert(
                    this.bonding.unbond(1000, { from: userAddress }),
                    'Permission: Not frozen'
                )
            })
        })
    ...
    describe('when step', function () {
        ...
        describe('after epoch lock cooldown', function () {
            ...
            it('has correct snapshots', async function () {
                expect(
                    await this.bonding.totalBondedAt(0)
                ).to.be.bignumber.equal(new BN(0))
                expect(
                    await this.bonding.totalBondedAt(1)
                ).to.be.bignumber.equal(
                    new BN(1000).mul(INITIAL_STAKE_MULTIPLE)
                )
                expect(
                    await this.bonding.totalBondedAt(2)
                ).to.be.bignumber.equal(
                    new BN(2000).mul(INITIAL_STAKE_MULTIPLE)
                )
                expect(
                    await this.bonding.totalBondedAt(3)
                ).to.be.bignumber.equal(
                    new BN(1000).mul(INITIAL_STAKE_MULTIPLE)
                )
            })
