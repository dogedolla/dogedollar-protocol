...
describe('Govern', function () {
    ...
    describe('vote', function () {
        describe('cant vote', function () {
            describe('when no stake', function () {
                it('reverts', async function () {
                    await expectRevert(
                        this.govern.vote(this.implB.address, APPROVE, {
                            from: userAddress,
                        }),
                        'Govern: Must have stake'
                    )
                })
            })
            ...
            describe('when not enough stake to propose', function () {
                beforeEach(async function () {
                    await this.govern.incrementBalanceOfE(
                        userAddress,
                        INITIAL_STAKE_MULTIPLE.muln(1)
                    )
                    await this.govern.incrementBalanceOfE(
                        userAddress2,
                        INITIAL_STAKE_MULTIPLE.muln(999)
                    )
            ...
            describe('when ended', function () {
                beforeEach(async function () {
                    await this.govern.incrementBalanceOfE(
                        userAddress,
                        INITIAL_STAKE_MULTIPLE.muln(1000)
                    )
                    await this.govern.incrementBalanceOfE(
                        userAddress2,
                        INITIAL_STAKE_MULTIPLE.muln(1000)
                    ...
                    await this.govern.vote(this.implB.address, APPROVE, {
                        from: userAddress2,
                    })
                ...
                it('is frozen', async function () {
                    expect(
                        await this.govern.statusOf(userAddress)
            ...
            describe('when fluid', function () {
                ...
                it('reverts', async function () {
                    await expectRevert(
                        ...
                        'Permission: Not frozen or locked'
                    )
                })
            })
        })
        ...
        describe('can vote', function () {
            ...
            describe('when vote', function () {
                beforeEach(async function () {
                    this.result = await this.govern.vote(
                        this.implB.address,
                        APPROVE,
                        { from: userAddress }
                    )
                ...
                it('sets vote counter correctly', async function () {
                    expect(
                        await this.govern.approveFor(this.implB.address)
                    ).to.be.bignumber.equal(
                        new BN(1000).mul(INITIAL_STAKE_MULTIPLE)
                    )
                    expect(
                        await this.govern.rejectFor(this.implB.address)
                    ).to.be.bignumber.equal(new BN(0))
                })

                it('is nominated', async function () {
                    expect(
                        await this.govern.isNominated(this.implB.address)
                    ).to.be.equal(true)
                })

                it('records vote', async function () {
                    expect(
                        await this.govern.recordedVote(
                            userAddress,
                            this.implB.address
                        )
                    ).to.be.bignumber.equal(APPROVE)
                })
                ...
                it('emits Vote event', async function () {
                    ...
                    expect(event.args.bonded).to.be.bignumber.equal(
                        new BN(1000).mul(INITIAL_STAKE_MULTIPLE)
                    )
                })
                ...
                it('emits Proposal event', async function () {
                    ...
                    expect(event.args.period).to.be.bignumber.equal(
                        new BN(VOTE_PERIOD)
                    )
                })
            })
            ...
            describe('when multiple vote', function () {
                beforeEach(async function () {
                    await this.govern.vote(this.implB.address, REJECT, {
                        from: userAddress2,
                    })
                    await this.govern.vote(this.implB.address, APPROVE, {
                        from: userAddress3,
                ...
                it('user is locked', async function () {
                    ...
                    expect(
                        await this.govern.statusOf(userAddress2)
                    ).to.be.bignumber.equal(new BN(2))
                    expect(
                        await this.govern.statusOf(userAddress3)
                    ).to.be.bignumber.equal(new BN(2))
                })
            ...
            describe('when revote', function () {
                beforeEach(async function () {
                    ...
                    await this.govern.vote(this.implB.address, UNDECIDED, {
                        from: userAddress3,
                    })
                    this.result = await this.govern.vote(
                        this.implB.address,
                ...
                it('records vote', async function () {
                    ...
                    ).to.be.bignumber.equal(UNDECIDED)
                })
    ...
    describe('commit', function () {
        beforeEach(async function () {
            ...
            await this.govern.incrementBalanceOfE(
                userAddress3,
                INITIAL_STAKE_MULTIPLE.muln(3500)
            )
        ...
        describe('ends successfully', function () {
            beforeEach(async function () {
                ...
                this.result = await this.govern.commit(this.implB.address, {
                    from: userAddress,
                })
                this.txHash = this.result.tx
            })
            ...
            it('is updated', async function () {
                expect(await this.govern.implementation()).to.be.equal(
                    this.implB.address
                )
                expect(
                    await this.govern.isInitialized(this.implB.address)
                ).to.be.equal(true)
            })
        ...
        describe('double commit - probably not possible in practice', function () {
            beforeEach(async function () {
                ...
                await this.govern.commit(this.implB.address, {
                    from: userAddress,
                })
            })
            ...
            it('reverts', async function () {
                await expectRevert(
                    ...
                    'Permission: Already initialized'
                )
            })
        })
    })
    ...
    describe('emergency commit', function () {
        ...
        describe('before nomination', function () {
            ...
            it('reverts', async function () {
                await expectRevert(
                    this.govern.emergencyCommit(this.implB.address, {
                        from: userAddress,
                    }),
                    'Govern: Not nominated'
                )
        ...
        describe('ended with not enough approve votes', function () {
            ...
            it('reverts', async function () {
                await expectRevert(
                    ...
                    'Govern: Must have super majority'
                )
            })
        })
        ...
        describe('ends successfully', function () {
            beforeEach(async function () {
                ...
                this.result = await this.govern.emergencyCommit(
                    this.implB.address,
                    { from: userAddress }
                )
                this.txHash = this.result.tx
