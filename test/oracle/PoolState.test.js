...
describe('PollState', function () {
    ...
    describe('decrementBalanceOfBonded', function () {
        describe('when called', function () {
            beforeEach('call', async function () {
                ...
                await this.setters.decrementBalanceOfBondedE(
                    userAddress,
                    100,
                    'decrementBalanceOfBondedE - 1'
                )
                await this.setters.decrementBalanceOfBondedE(
                    userAddress,
                    100,
                    'decrementBalanceOfBondedE - 2'
                )
            })
        ...
        describe('when called erroneously', function () {
            ...
            it('reverts', async function () {
                await expectRevert(
                    this.setters.decrementBalanceOfBondedE(
                        200,
    ...
    describe('incrementBalanceOfClaimable', function () {
        describe('when called', function () {
            beforeEach('call', async function () {
                await this.setters.incrementBalanceOfClaimableE(
                    userAddress,
                    100
                )
                await this.setters.incrementBalanceOfClaimableE(
            ...
            it('increments balance of claimable for user', async function () {
                expect(
                    await this.setters.balanceOfClaimable(userAddress)
                ).to.be.bignumber.equal(new BN(200))
            })

            it('increments total claimable', async function () {
                expect(
                    await this.setters.totalClaimable()
                ).to.be.bignumber.equal(new BN(200))
            })
        })
    })
    ...
    describe('decrementBalanceOfClaimable', function () {
        describe('when called', function () {
            beforeEach('call', async function () {
                ...
                await this.setters.decrementBalanceOfClaimableE(
                    userAddress,
                    100,
                    'decrementBalanceOfClaimableE - 1'
                )
                await this.setters.decrementBalanceOfClaimableE(
                    userAddress,
                    100,
                    'decrementBalanceOfClaimableE - 2'
                )
            })
        ...
        describe('when called erroneously', function () {
            ...
            it('reverts', async function () {
                await expectRevert(
                    this.setters.decrementBalanceOfClaimableE(
                        200,
    ...
    describe('incrementBalanceOfPhantom', function () {
        describe('when called', function () {
            ...
            it('increments balance of phantom for user', async function () {
                expect(
                    await this.setters.balanceOfPhantom(userAddress)
                ).to.be.bignumber.equal(new BN(200))
            })

            it('increments total phantom', async function () {
                expect(await this.setters.totalPhantom()).to.be.bignumber.equal(
                    new BN(200)
                )
            })
        })
    ...
    describe('decrementBalanceOfPhantom', function () {
        describe('when called', function () {
            beforeEach('call', async function () {
                ...
                await this.setters.decrementBalanceOfPhantomE(
                    userAddress,
                    100,
                    'decrementBalanceOfPhantomE - 1'
                )
                await this.setters.decrementBalanceOfPhantomE(
                    userAddress,
                    100,
                    'decrementBalanceOfPhantomE - 2'
                )
            })
        ...
        describe('when called erroneously', function () {
            ...
            it('reverts', async function () {
                await expectRevert(
                    this.setters.decrementBalanceOfPhantomE(
                        200,
    ...
    describe('rewarded', function () {
        describe('no user', function () {
            ...
            it('reward display correctly', async function () {
                expect(
                    await this.setters.balanceOfRewarded(userAddress)
                ).to.be.bignumber.equal(new BN(0))
                expect(
                    await this.setters.totalRewarded()
        ...
        describe('multiple user', function () {
            ...
            describe('when called', function () {
                ...
                it('reward display correctly', async function () {
                    expect(
                        await this.setters.balanceOfRewarded(userAddress)
                    ).to.be.bignumber.equal(new BN(125))
                    expect(
                        await this.setters.balanceOfRewarded(userAddress2)
                    ).to.be.bignumber.equal(new BN(375))
                    expect(
                        await this.setters.totalRewarded()
                    ).to.be.bignumber.equal(new BN(500))
                })
