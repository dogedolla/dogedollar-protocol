...
describe("Pool", function () {
  ...
  beforeEach(async function () {
    this.dao = await MockSettableDAO.new({
      from: ownerAddress,
      gas: 8000000,
    });
    await this.dao.set(1);
    this.dollar = await MockToken.new("Dynamic Set Dollar", "DOGEDOLA", 18, {
      from: ownerAddress,
      gas: 8000000,
    });
  ...
  describe("frozen", function () {
    ...
    describe("when deposit", function () {
      beforeEach(async function () {
        ...
        await this.univ2.approve(this.pool.address, 1000, {
          from: userAddress,
        });

        this.result = await this.pool.deposit(1000, {
          from: userAddress,
        });
    ...
    describe("when withdraw", function () {
      describe("simple", function () {
        beforeEach(async function () {
          ...
          this.result = await this.pool.withdraw(1000, {
            from: userAddress,
          });
          this.txHash = this.result.tx;
        });
      ...
      describe("too much", function () {
        beforeEach(async function () {
          ...
          await this.univ2.approve(this.pool.address, 10000, {
            from: userAddress1,
          });
    ...
    describe("when claim", function () {
      ...
      describe("simple", function () {
        beforeEach(async function () {
          this.result = await this.pool.claim(1000, {
            from: userAddress,
          });
          this.txHash = this.result.tx;
        });
    ...
    describe("when bond", function () {
      describe("no reward", function () {
        describe("simple", function () {
          beforeEach(async function () {
            ...
            this.result = await this.pool.bond(1000, {
              from: userAddress,
            });
            this.txHash = this.result.tx;
          });
        ...
        describe("partial", function () {
          beforeEach(async function () {
            ...
            this.result = await this.pool.bond(500, {
              from: userAddress,
            });
            this.txHash = this.result.tx;
          });
        ...
        describe("multiple", function () {
          beforeEach(async function () {
            ...
            await this.univ2.faucet(userAddress, 1000);
            await this.univ2.approve(this.pool.address, 800, {
              from: userAddress,
            });
            await this.pool.deposit(800, { from: userAddress });
      ...
      describe("with reward", function () {
        ...
        describe("multiple without reward first", function () {
          ...
          it("updates users balances", async function () {
            expect(await this.dollar.balanceOf(userAddress)).to.be.bignumber.equal(new BN(0));
            expect(await this.pool.balanceOfRewarded(userAddress1)).to.be.bignumber.equal(
              new BN(600).mul(INITIAL_STAKE_MULTIPLE),
            );
            expect(await this.pool.balanceOfPhantom(userAddress1)).to.be.bignumber.equal(
              new BN(600).mul(INITIAL_STAKE_MULTIPLE),
            );
            expect(await this.pool.balanceOfRewarded(userAddress2)).to.be.bignumber.equal(
              new BN(400).mul(INITIAL_STAKE_MULTIPLE),
            );
            expect(await this.pool.balanceOfPhantom(userAddress2)).to.be.bignumber.equal(
              new BN(400).mul(INITIAL_STAKE_MULTIPLE),
            );
            expect(await this.pool.balanceOfRewarded(userAddress)).to.be.bignumber.equal(new BN(0));
            expect(await this.pool.balanceOfPhantom(userAddress)).to.be.bignumber.equal(
              new BN(1000).mul(INITIAL_STAKE_MULTIPLE),
            );
          });

          it("updates dao balances", async function () {
            expect(await this.dollar.balanceOf(this.pool.address)).to.be.bignumber.equal(
              new BN(1000).mul(INITIAL_STAKE_MULTIPLE),
            );
    ...
    describe("when unbond", function () {
      describe("without reward", function () {
        ...
        describe("simple", function () {
          beforeEach(async function () {
            this.result = await this.pool.unbond(new BN(1000), {
              from: userAddress,
            });
            this.txHash = this.result.tx;
          });
        ...
        describe("partial", function () {
          beforeEach(async function () {
            this.result = await this.pool.unbond(new BN(800), {
              from: userAddress,
            });
            this.txHash = this.result.tx;
          });
      ...
      describe("with reward", function () {
        ...
        describe("partial", function () {
          ...
          it("updates users balances", async function () {
            ...
            expect(await this.pool.balanceOfPhantom(userAddress)).to.be.bignumber.equal(
              new BN(200).mul(INITIAL_STAKE_MULTIPLE),
            );
          });
        ...
        describe("multiple", function () {
          ...
          it("updates users balances", async function () {
            ...
            expect(await this.pool.balanceOfPhantom(userAddress1)).to.be.bignumber.equal(
              new BN(600).mul(INITIAL_STAKE_MULTIPLE).addn(600),
            );
            ...
            expect(await this.pool.balanceOfPhantom(userAddress2)).to.be.bignumber.equal(
              new BN(400).mul(INITIAL_STAKE_MULTIPLE).addn(400),
            );
          });
          ...
          it("updates dao balances", async function () {
            ...
            expect(await this.pool.totalPhantom()).to.be.bignumber.equal(
              new BN(1200).mul(INITIAL_STAKE_MULTIPLE).addn(1000),
            );
          });
        ...
        describe("potential subtraction underflow", function () {
          beforeEach(async function () {
            ...
            await this.pool.unbond(new BN(1000), {
              from: userAddress,
            });
            await this.pool.bond(new BN(1000), {
              from: userAddress,
            });
            await this.pool.unbond(new BN(600), {
              from: userAddress,
            });

            this.result = await this.pool.unbond(new BN(200), {
              from: userAddress,
            });
            this.txHash = this.result.tx;
          });
          ...
          it("updates users balances", async function () {
            ...
            expect(await this.pool.balanceOfRewarded(userAddress)).to.be.bignumber.equal(new BN(0));
            expect(await this.pool.balanceOfPhantom(userAddress)).to.be.bignumber.equal(
              new BN(200).mul(INITIAL_STAKE_MULTIPLE).addn(296),
            );
            expect(await this.pool.balanceOfClaimable(userAddress1)).to.be.bignumber.equal(new BN(0));
            ...
            expect(await this.pool.balanceOfPhantom(userAddress2)).to.be.bignumber.equal(
              new BN(500).mul(INITIAL_STAKE_MULTIPLE).addn(500),
            );
          });
          ...
          it("updates dao balances", async function () {
            ...
            expect(await this.pool.totalPhantom()).to.be.bignumber.equal(
              new BN(1300).mul(INITIAL_STAKE_MULTIPLE).addn(1396),
            );
          });
    ...
    describe("when provide", function () {
      ...
      describe("simple", function () {
        ...
        it("is fluid", async function () {
          // // allows adding liquidity: https://github.com/dynamicsetdollar/dogedola-protocol/commit/797a13fd23ba645c8e7b6debf9253f671814be47
          expect(await this.pool.statusOf(userAddress)).to.be.bignumber.equal(FLUID);
        });
  ...
  describe("fluid", function () {
    ...
    describe("when provide", function () {
      it("allows providing", async function () {
        ...
        await this.univ2.set(1000, 1000, 10);
        const result = await this.pool.provide(1000, { from: userAddress });
    ...
    describe("when unbond", function () {
      beforeEach(async function () {
        this.result = await this.pool.unbond(new BN(500), {
          from: userAddress,
        });
        this.txHash = this.result.tx;
      });
  ...
  describe("when emergency withdraw", function () {
    ...
    describe("as dao", function () {
      beforeEach(async function () {
        await this.pool.set(mockDao, this.dollar.address, this.univ2.address);
        await this.pool.emergencyWithdraw(this.univ2.address, 1000, {
          from: mockDao,
        });
        await this.pool.emergencyWithdraw(this.dollar.address, 1000, {
          from: mockDao,
        });
      });
    ...
    describe("as not dao", function () {
      it("reverts", async function () {
        await expectRevert(
          this.pool.emergencyWithdraw(this.univ2.address, 1000, {
            from: userAddress,
          }),
          "Not dao",
        );
