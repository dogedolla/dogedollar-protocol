    ...
    describe('step', function () {
        ...
        describe('after advance without trade', function () {
            ...
            it('is uninitialized', async function () {
                expect(await priceForToBN(this.oracle)).to.be.bignumber.equal(
                    cents(100)
                )
                expect(await this.oracle.isInitialized()).to.be.equal(false)
                expect(await this.oracle.cumulative()).to.be.bignumber.equal(
                    new BN(0)
                )
            ...
            describe('price of 1', function () {
                describe('same block', function () {
                    ...
                    it('is initialized', async function () {
                        expect(
                            await priceForToBN(this.oracle)
                        ).to.be.bignumber.equal(cents(100))
                        expect(await this.oracle.isInitialized()).to.be.equal(
                            true
                        )
                        expect(
                            await this.oracle.cumulative()
            ...
            describe('price of 1', function () {
                describe('same block', function () {
                    ...
                    it('is initialized', async function () {
                        expect(
                            await priceForToBN(this.oracle)
                        ).to.be.bignumber.closeTo(cents(100), EPSILON)
                        expect(await this.oracle.isInitialized()).to.be.equal(
                            true
                        )
                        expect(
                            await this.oracle.cumulative()
                        ).to.be.bignumber.equal(uint112s(this.timediff))
                        expect(
                            await this.oracle.timestamp()
                        ).to.be.bignumber.equal(this.timestamp)
                        expect(
            ...
            describe('price greater than 1', function () {
                describe('same block', function () {
                    ...
                    it('is initialized', async function () {
                        expect(
                            await priceForToBN(this.oracle)
                        ).to.be.bignumber.closeTo(cents(110), EPSILON)
                        expect(await this.oracle.isInitialized()).to.be.equal(
                            true
                        )
                        expect(
                            ...
                            uint112s(this.timediff, 1100000, 1000000)
                        )
                        expect(
                            await this.oracle.timestamp()
                        ).to.be.bignumber.equal(this.timestamp)
                ...
                describe('different prices', function () {
                    ...
                    it('is initialized', async function () {
                        ...
                        expect(
                            ...
                            uint112s(this.timediff, 1150000, 1000000)
                        )
                        expect(
                            await this.oracle.timestamp()
                        ).to.be.bignumber.equal(this.timestamp)
        ...
        describe('after multiple advances with multiple trades', function () {
            ...
            describe('price greater than 1', function () {
                ...
                describe('different prices', function () {
                    ...
                    it('is initialized', async function () {
                        const begin = uint112s(
                            this.middle.sub(this.initialized).toNumber(),
                            1150000,
                            1000000
                        )
                        const end = uint112s(
                            this.timestamp.sub(this.middle).toNumber(),
                            1050000,
                            1000000
                        )
                        ...
                        expect(
                            await priceForToBN(this.oracle)
                        ).to.be.bignumber.closeTo(cents(105), EPSILON)
                        expect(await this.oracle.isInitialized()).to.be.equal(
                            true
                        )
                        expect(
                            await this.oracle.cumulative()
                        ).to.be.bignumber.equal(begin.add(end))
                        expect(
                            await this.oracle.timestamp()
                        ).to.be.bignumber.equal(this.timestamp)
                        expect(
        ...
        describe('after many advances', function () {
            describe('different prices', function () {
                ...
                it('is initialized', async function () {
                    expect(
                        await priceForToBN(this.oracle)
                    ).to.be.bignumber.closeTo(cents(95), EPSILON)
        ...
        describe('current reserve too low', function () {
            describe('long before', function () {
                ...
                it('is initialized', async function () {
                    expect(
                        await priceForToBN(this.oracle)
                    ).to.be.bignumber.closeTo(cents(120), EPSILON)
                    expect(await this.oracle.latestValid()).to.be.equal(false)
                    expect(await this.oracle.isInitialized()).to.be.equal(true)
                    expect(
                        await this.oracle.cumulative()
                    ).to.be.bignumber.equal(uint112s(this.timediff, 3000, 2500))
                    expect(await this.oracle.timestamp()).to.be.bignumber.equal(
                        this.timestamp
                    )
                    expect(await this.oracle.reserve()).to.be.bignumber.equal(
