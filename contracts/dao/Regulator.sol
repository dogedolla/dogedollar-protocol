...
contract Regulator is Comptroller {
    ...
    function step() internal {
        (Decimal.D256 memory price, Decimal.D256 memory CDOGEDOLAPrice) = oracleCapture();

        setPrices(price, CDOGEDOLAPrice);
    ...
    function contraction(Decimal.D256 memory price) private {
        (uint256 newDOGEDOLASupply) = contractionIncentives(price);

        emit ContractionIncentives(epoch(), price.value, newDOGEDOLASupply);
    }
    ...
    function oracleCapture() private returns (Decimal.D256 memory, Decimal.D256 memory) {
        ...
        (Decimal.D256 memory CDOGEDOLAPrice, bool contractionValid) = contractionOracle().capture();
        if (!contractionValid) {
            CDOGEDOLAPrice = price;
        }

        return (price, CDOGEDOLAPrice);
    }
}
