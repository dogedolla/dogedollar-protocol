...
contract Comptroller is Setters {
    ...
    function setPrices(Decimal.D256 memory price, Decimal.D256 memory CDOGEDOLAPrice) internal {
        _state13.price = price;
        _state17.CDOGEDOLAPrice = CDOGEDOLAPrice;
    ...
    function contractionIncentives(Decimal.D256 memory price) internal returns (uint256) {
        ...
        uint256 redeemable = totalCDOGEDOLARedeemable();
        if (redeemable != 0) {
            clearCDOGEDOLARedeemable();
        }

        // accrue interest on CDOGEDOLA
        uint256 currentMultiplier = globalInterestMultiplier();
        Decimal.D256 memory interest = Constants.maxCDOGEDOLABondingRewards();
    ...
    function increaseSupply(uint256 newSupply) internal returns (uint256, uint256) {
        ...
        // cDOGEDOLA redemption logic
        uint256 newCDOGEDOLARedeemable = 0;
        uint256 outstanding = maxCDOGEDOLAOutstanding();
        uint256 redeemable = totalCDOGEDOLARedeemable().sub(totalCDOGEDOLARedeemed());
        if (redeemable < outstanding) {
            uint256 newRedeemable = newSupply.mul(Constants.getCDOGEDOLARedemptionRatio()).div(100);
            uint256 newRedeemableCap = outstanding.sub(redeemable);

            newCDOGEDOLARedeemable = newRedeemableCap > newRedeemable ? newRedeemableCap : newRedeemable;

            incrementTotalCDOGEDOLARedeemable(newCDOGEDOLARedeemable);
        }

        // remaining is for DAO
        uint256 rewards = poolReward.add(treasuryReward).add(newCDOGEDOLARedeemable);
        ...
        return (newCDOGEDOLARedeemable, amount.add(rewards));
    }
