...
/*
    ...
    Copyright 2022 Dynamic Doge Devs, based on the works of the Empty Set Squad
...
// File @openzeppelin/contracts/token/ERC20/ERC20Burnable.sol@v2.5.1
...
// File @openzeppelin/contracts/access/Roles.sol@v2.5.1
...
// File @openzeppelin/contracts/access/roles/MinterRole.sol@v2.5.1
...
library Constants {
    /* Chain */
    uint256 private constant CHAIN_ID = 2000; // Dogechain

    /* Bootstrapping */
    uint256 private constant BOOTSTRAPPING_PERIOD = 24; // 24 epochs - 2 days
    uint256 private constant BOOTSTRAPPING_PRICE = 154e16; // 1.54 USDT (targeting 4.5% inflation)

    /* Oracle */
    address private constant USDT = address(0xE3F5a90F9cb311505cd691a46596599aA1A0AD7D);
    uint256 private constant ORACLE_RESERVE_MINIMUM = 1e7; // 10,000 USDT //adjust 1e10 after testing
    ...
    uint256 private constant EPOCH_START = 1660910400;
    ...
    /* DAO */
    uint256 private constant ADVANCE_INCENTIVE = 10e18; // 10 DOGEDOLA
    ...
    /* Market */
    uint256 private constant COUPON_EXPIRATION = 180; // 15 Days
    ...
    uint256 private constant SUPPLY_CHANGE_DIVISOR = 100e18; // 100 > Max expansion at $2
    ...
    uint256 private constant ORACLE_POOL_RATIO = 80; // 80%
...
// File @yode/v2-core/contracts/interfaces/IYodedexPair.sol@v1.0.1
...
// File @yode/lib/contracts/libraries/Babylonian.sol@v1.1.4
...
// File @yode/lib/contracts/libraries/FixedPoint.sol@v1.1.4
...
// File contracts/external/YodeOracleLibrary.sol

pragma solidity >=0.5.0;


// library with helper methods for oracles that are concerned with computing average prices
library YodeOracleLibrary {
    ...
    returns (uint price0Cumulative, uint price1Cumulative, uint32 blockTimestamp) {
        ...
        price0Cumulative = IYodedexPair(pair).price0CumulativeLast();
        price1Cumulative = IYodedexPair(pair).price1CumulativeLast();

        // if time has elapsed since the last update on the pair, mock the accumulated price values
        (uint112 reserve0, uint112 reserve1, uint32 blockTimestampLast) = IYodedexPair(pair).getReserves();
...
library YodedexLibrary {
    ...
    {
        require(tokenA != tokenB, "YodedexLibrary: IDENTICAL_ADDRESSES");
        (token0, token1) = tokenA < tokenB
            ? (tokenA, tokenB)
            : (tokenB, tokenA);
        require(token0 != address(0), "YodedexLibrary: ZERO_ADDRESS");
    }
            ...
            uint256(
                keccak256(
                    abi.encodePacked(
                        ...
                        hex"746cdd6c24c5f1496602686ccbb848d5d9c8e5ccc408250c4cc8be8331de6857" // init code hash
                    )
                )
            )
        );
    ...
    function getReserves(
        ...
        (uint256 reserve0, uint256 reserve1, ) = IYodedexPair(
            pairFor(factory, tokenA, tokenB)
        ).getReserves();
    ...
    function quote(
        ...
        require(amountA > 0, "YodedexLibrary: INSUFFICIENT_AMOUNT");
        require(
            reserveA > 0 && reserveB > 0,
            "YodedexLibrary: INSUFFICIENT_LIQUIDITY"
        );
    ...
    function getAmountOut(
        ...
        require(amountIn > 0, "YodedexLibrary: INSUFFICIENT_INPUT_AMOUNT");
        require(
    ...
    function getAmountIn(
        ...
        require(amountOut > 0, "YodedexLibrary: INSUFFICIENT_OUTPUT_AMOUNT");
        require(
            reserveIn > 0 && reserveOut > 0,
            "YodedexLibrary: INSUFFICIENT_LIQUIDITY"
        );
    ...
    function getAmountsOut(
        ...
        require(path.length >= 2, "YodedexLibrary: INVALID_PATH");
...
contract Oracle is IOracle {
    ...
    address private constant YODE_FACTORY = address(0xAaA04462e35f3e40D798331657cA015169e005d7);

    address internal _dao;
    address internal _dollar;
    bool internal _initialized;
    IYodedexPair internal _pair;
    ...
    constructor (address dollar, address dao, address pair) public {
        _dao = dao;
        _dollar = dollar;
        _pair = IYodedexPair(pair);
    }
    ...
    function updatePrice() private returns (Decimal.D256 memory) {
        ...
        YodeOracleLibrary.currentCumulativePrices(address(_pair));
    ...
    function usdt() internal pure returns (address) {
...
contract PoolGetters is PoolState {
     ...
     */
     ...
     address internal dollarAddress;
    ...
    function yodeLp() public view returns (IERC20) {
        return IERC20(pairAddress);
    }
...
// File contracts/oracle/Liquidity.sol
...
contract Liquidity is PoolGetters {
    ...
    function addLiquidity(uint256 dollarAmount) internal returns (uint256, uint256) {
        ...
        uint256 usdtAmount = (reserveA == 0 && reserveB == 0) ?
             dollarAmount :
             YodedexLibrary.quote(dollarAmount, reserveA, reserveB);

        address pair = address(yodeLp());
        IERC20(dollar).transfer(pair, dollarAmount);
        IERC20(usdt).transferFrom(msg.sender, pair, usdtAmount);
        return (usdtAmount, IYodedexPair(pair).mint(address(this)));
    }

    // overridable for testing
    function getReserves(address tokenA, address tokenB) internal view returns (uint reserveA, uint reserveB) {
        (address token0,) = YodedexLibrary.sortTokens(tokenA, tokenB);
        (uint reserve0, uint reserve1,) = IYodedexPair(YodedexLibrary.pairFor(YODE_FACTORY, tokenA, tokenB)).getReserves();
...
contract Pool is PoolSetters, Liquidity {
    ...
    constructor(address _dollar, address _yodeLp, address _dao) public 
        {
            PoolGetters.daoAddress = _dao;
            PoolGetters.dollarAddress = _dollar;
            PoolGetters.pairAddress = _yodeLp;
         }
    ...
    event Provide(address indexed account, uint256 value, uint256 lessUsdt, uint256 newyodeLp);

    function deposit(uint256 value) external onlyFrozen(msg.sender) notPaused {
        yodeLp().transferFrom(msg.sender, address(this), value);
    ...
    function withdraw(uint256 value) external onlyFrozen(msg.sender) {
        yodeLp().transfer(msg.sender, value);
    ...
    function provide(uint256 value) external notPaused {
        ...
        (uint256 lessUsdt, uint256 newyodeLp) = addLiquidity(value);

        uint256 totalRewardedWithPhantom = totalRewarded().add(totalPhantom()).add(value);
        uint256 newPhantomFromBonded = totalRewardedWithPhantom.mul(newyodeLp).div(totalBonded());

        incrementBalanceOfBonded(msg.sender, newyodeLp);
        incrementBalanceOfPhantom(msg.sender, value.add(newPhantomFromBonded));


        balanceCheck();

        emit Provide(msg.sender, value, lessUsdt, newyodeLp);
    }
    ...
    function balanceCheck() private {
        Require.that(
            yodeLp().balanceOf(address(this)) >= totalStaged().add(totalBonded()),
            FILE,
...
// File @openzeppelin/upgrades/contracts/utils/Address.sol@v2.8.0
...
contract Getters is State {
    ...
    function name() public view returns (string memory) {
        return "Staked Dynamic Doge Dollar";
    }
...
// File contracts/dao/Deployer.sol
...
contract Deployer2 is State, Permission, Upgradeable {
    function initialize() initializer public {
       // _state.provider.oracle = new Oracle(address(0x0FA3242e471Da9f532A5c7e9Bc073a93A08Ab4D6));
        oracle().setup();
    }
...
contract Deployer3 is State, Permission, Upgradeable {
    function initialize() initializer public {
        //_state.provider.pool = address(new Pool(address(0x0FA3242e471Da9f532A5c7e9Bc073a93A08Ab4D6), address(0x75828156b12e4783d4f9040e0310a75262e61604), address(msg.sender))   
