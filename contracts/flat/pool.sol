...
library YodeLibrary {
    ...
    function sortTokens(address tokenA, address tokenB) internal pure returns (address token0, address token1) {
        require(tokenA != tokenB, 'YodeLibrary: IDENTICAL_ADDRESSES');
        (token0, token1) = tokenA < tokenB ? (tokenA, tokenB) : (tokenB, tokenA);
        require(token0 != address(0), 'YodeLibrary: ZERO_ADDRESS');
    }
    ...
    function getReserves(address factory, address tokenA, address tokenB) internal view returns (uint reserveA, uint reserveB) {
        ...
        (uint reserve0, uint reserve1,) = IYodedexPair(pairFor(factory, tokenA, tokenB)).getReserves();
        (reserveA, reserveB) = tokenA == token0 ? (reserve0, reserve1) : (reserve1, reserve0);
    }

    // given some amount of an asset and pair reserves, returns an equivalent amount of the other asset
    function quote(uint amountA, uint reserveA, uint reserveB) internal pure returns (uint amountB) {
        require(amountA > 0, 'YodeLibrary: INSUFFICIENT_AMOUNT');
        require(reserveA > 0 && reserveB > 0, 'YodeLibrary: INSUFFICIENT_LIQUIDITY');
        amountB = amountA.mul(reserveB) / reserveA;
    }
}
// File: @yodeswap/v2-core/contracts/interfaces/IYodedexPair.sol
...
// File: localhost/contracts/oracle/PoolState.sol

/*
    Copyright 2022 DOGEDOLA Devs, based on the works of the Empty Set Squad
...
// File: localhost/contracts/oracle/PoolGetters.sol
...
// File: localhost/contracts/oracle/PoolSetters.sol
...
/*
    ...
    limitations under the License.
*/
// File: localhost/contracts/oracle/Liquidity.sol
...
contract Liquidity is PoolGetters {
    ...
    function addLiquidity(uint256 dollarAmount) internal returns (uint256, uint256) {
        ...
        uint256 usdtAmount = (reserveA == 0 && reserveB == 0) ?
             dollarAmount :
             YodeLibrary.quote(dollarAmount, reserveA, reserveB);
    ...
    function getReserves(address tokenA, address tokenB) internal view returns (uint reserveA, uint reserveB) {
        (address token0,) = YodeLibrary.sortTokens(tokenA, tokenB);
        (uint reserve0, uint reserve1,) = IYodedexPair(YodeLibrary.pairFor(YODE_FACTORY, tokenA, tokenB)).getReserves();
        (reserveA, reserveB) = tokenA == token0 ? (reserve0, reserve1) : (reserve1, reserve0);
    }
}
...
// File: localhost/contracts/Constants.sol
...
library Constants {
    /* Chain */
    uint256 private constant CHAIN_ID = 56; // Mainnet BSC
    ...
    /* Oracle */
    address private constant USDT = address(0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56);
    uint256 private constant ORACLE_RESERVE_MINIMUM = 1000e19; // 10,000 USDT
    ...
    /* DAO */
    uint256 private constant ADVANCE_INCENTIVE = 10e18; // 150 DOGEDOLA
    ...
    uint256 private constant ORACLE_POOL_RATIO = 77; // 80%
...
// File: localhost/contracts/external/Require.sol
...
// File: localhost/contracts/token/IDollar.sol
...
// File: localhost/contracts/oracle/Pool.sol
...
contract Pool is PoolSetters, Liquidity {
    ...
    constructor(address dollar, address yodelp) public {
        _state.provider.dao = IDAO(msg.sender);
        _state.provider.dollar = IDollar(dollar);
        _state.provider.yodelp = IERC20(yodelp);
    }
