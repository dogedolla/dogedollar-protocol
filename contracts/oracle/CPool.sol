...
import "./CPoolSetters.sol";
import "./CLiquidity.sol";

contract CPool is CPoolSetters, CLiquidity {
    ...
    function claim(uint256 value) external onlyFrozen(msg.sender) {
        cdogedola().transfer(msg.sender, value);
    ...
    modifier onlyFrozen(address account) {
        Require.that(
            statusOf(account) == CPoolAccount.Status.Frozen,
            FILE,
            "Not frozen"
        );
