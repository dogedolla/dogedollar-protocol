...
contract CDOGEDOLAOracle is IOracle {
    using Decimal for Decimal.D256;

    bytes32 private constant FILE = "Oracle";
    address private constant PANCAKESWAP_FACTORY = address(0xC0AEe478e3658e2610c5F7A4A2E1777cE9e4f2Ac); // Sushi Factory Address
    ...
    function setup() public onlyDao {
        _pair = IUniswapV2Pair(
            IUniswapV2Factory(PANCAKESWAP_FACTORY).getPair(Constants.getContractionDollarAddress(), busd())
        );
        (address token0, address token1) = (_pair.token0(), _pair.token1());
        _index = Constants.getContractionDollarAddress() == token0 ? 0 : 1;
        Require.that(_index == 0 || Constants.getContractionDollarAddress() == token1, FILE, "CDOGEDOLA not found");
    }
