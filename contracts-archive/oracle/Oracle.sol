...
contract Oracle is IOracle {
    ...
    address private constant SUSHISWAP_FACTORY = address(0xcA143Ce32Fe78f1f7019d7d551a6402fC5350c73); // Sushi Factory Address
    ...
    function setup() public onlyDao {
        _pair = IUniswapV2Pair(IUniswapV2Factory(SUSHISWAP_FACTORY).getPair(Constants.getDollarAddress(), busd()));

        (address token0, address token1) = (_pair.token0(), _pair.token1());
        _index = Constants.getDollarAddress() == token0 ? 0 : 1;

        Require.that(_index == 0 || Constants.getDollarAddress() == token1, FILE, "PANCAKE not found");
    }
